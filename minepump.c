...
void MethaneMonitoring_Body (void) {
  ...
  else if(MS>MS_L1 && MS<MS_L2){level=Alarm1;
                               MI_write(LvlMeth, level);}


  else if(MS>MS_L2){level=Alarm2;
                    MI_write(LvlMeth, level);}
  sem_post(&synchro);
...
void *PumpCtrl_Body(void *no_argument) {
  ...
  for (;;) {
      ...
      if(niveau_alarme==Normal){alarme=0;}
      else{alarme=1;}
      msg_box_send(mbox_alarm,(char*)&alarme);
    


     if(niveau_alarme==Alarm2){cmd=0;}
     else{
          niveau_eau=MI_read(LvlWater);
          if(niveau_eau==1){cmd=1;}
          else if(niveau_eau==0){cmd=0;}
          }
         CmdPump(cmd);   
         CmdAlarm(cmd);
  }
...
int main(void) {
  ...
  struct timespec W_period;
  W_period.tv_nsec = 250 * 1000 * 1000;
  W_period.tv_sec  = 0 ;

  create_periodic_task (W_period, WaterLevelMonitoring_Body);
  printf ("Task created\n");
  /* Create task MethaneMonitoring_Task */
  struct timespec M_period;
  M_period.tv_nsec = 100 * 1000 * 1000;
  M_period.tv_sec  = 0 ;

  create_periodic_task (M_period, MethaneMonitoring_Body);
  printf ("Task created\n");
